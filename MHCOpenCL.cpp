...
void BayerKernel(uchar* src, uchar* out, const int width, const int height)
{
	...
	int OutBufferSize = sizeof(uchar) * width * height * 3;


	//Open MHCFilter.cl file and store it in local string
	char *source_str = (char*)malloc(MAX_SOURCE_SIZE);
	size_t source_size;
	FILE *fp;
	errno_t errnum = fopen_s(&fp, "MHCFilter.cl", "r");
	fseek(fp, 0, SEEK_END);
	...
	//Create the Kernel to call the function mhc_kernel_uchar
	...
	ret = clEnqueueReadBuffer(command_queue, img_Output, CL_TRUE, NULL, OutBufferSize, out, 0, NULL, NULL);
